...
public class FileOrganizerService
{
    ...
    public FileLogEntry Organize(string sourcePath, ClassificationResult classification, AppSettings settings) =>
        OrganizeCore(sourcePath, sourcePath, classification, settings);

    /// <summary>
    /// מתייק/מעביר מחדש קובץ ל"לקוח"/"סוג" שהמשתמש בחר ידנית - משמש הן לתיקון קובץ שכבר
    /// תויק לא נכון (Reclassify מעביר מ-<see cref="FileLogEntry.NewPath"/> הקיים) והן לתיוק
    /// קובץ שחיכה ב"ממתין לבדיקה" (Reclassify מעביר מ-<see cref="FileLogEntry.OriginalPath"/>,
    /// כי הוא מעולם לא זז). יומן הפעילות עדיין מציג את הנתיב המקורי האמיתי, לא את המיקום
    /// הזמני שממנו הקובץ הועבר עכשיו בפועל.
    /// </summary>
    public FileLogEntry Reclassify(FileLogEntry entry, ClassificationResult classification, AppSettings settings)
    {
        var currentPath = entry.NewPath ?? entry.OriginalPath;
        return OrganizeCore(currentPath, entry.OriginalPath, classification, settings);
    }

    private FileLogEntry OrganizeCore(string currentPath, string originalPathForLog, ClassificationResult classification, AppSettings settings)
    {
        var entry = new FileLogEntry
        {
            OriginalPath = originalPathForLog,
