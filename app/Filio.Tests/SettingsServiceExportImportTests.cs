...
/// בלי לגעת ב-%AppData%\Filio האמיתי (בניגוד ל-Load/Save שמשתמשים בנתיב קבוע).
/// </summary>
public class SettingsServiceExportImportTests : IDisposable
{
    private readonly string _sandbox = Path.Combine(Path.GetTempPath(), "FilioSettingsIoTests_" + Guid.NewGuid().ToString("N"));
