...
{
    private readonly string _sandbox = Path.Combine(Path.GetTempPath(), "FileFoxTests_" + Guid.NewGuid().ToString("N"));
    ...
    public void OrganizeInstaller_RoutesToDedicatedInstallersFolderByYear()
    {
        ...
        var source = CreateSourceFile("FileFoxSetup.exe");
        var settings = NewSettings();
        ...
        Assert.Equal(Path.Combine(expectedDir, "FileFoxSetup.exe"), entry.NewPath);
        Assert.True(File.Exists(entry.NewPath!));
